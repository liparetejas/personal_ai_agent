"""
Interactive Pizza Ordering Assistant - Batak (HTTP API Version)
An interactive AI assistant that helps order Domino's pizza using the remote MCPizza HTTP API.
...
class HTTPMCPClient:
    """Simple HTTP MCP client for calling remote MCPizza API"""

    def __init__(self, api_url="https://mcpizza.vercel.app/api/mcp"):
...
async def handle_pizza_request_with_gemini(user_request: str, mcp_client, gemini_client, system_instruction: str):
    """
    Handle pizza ordering requests using Gemini with HTTP MCP client.

    Args:
        user_request: Natural language pizza order request from user
        mcp_client: HTTP MCP client instance
    ...
    response = await gemini_client.aio.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            ...
            temperature=0.7,
            tools=[mcp_client],  # HTTP MCP client available to Gemini
        ),
    )
...
async def main():
    print("🍕 Starting MCPizza HTTP API Client...")
    print("📡 Using remote API: https://mcpizza.vercel.app/api/mcp\n")
    ...
    system_instruction = """Your name is Batak. You are a personal assistant for Tejas who helps him order Domino's pizza. You use MCP tools to look up Domino's Pizza stores, menus, build orders, and generate order summarie to show to the user.
Respond clearly and fully to user queries about ordering pizza, including confirming found addresses and menus.
...
"""
    ...
    try:
        ...
        if "error" in tools_response:
            print(f"⚠️  Warning: Could not connect to MCP API: {tools_response['error']}")
            print("Continuing anyway...\n")
        else:
            print("✅ MCPizza HTTP API Connected")
            print(f"🍕 API Status: {tools_response.get('name', 'MCPizza')} v{tools_response.get('version', '1.0')}\n")
